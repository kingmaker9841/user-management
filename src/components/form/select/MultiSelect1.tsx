...
const MultiSelectComponent = (props: any) => {
  ...
  React.useEffect(() => {
    if (members) setselectedName(members)
  }, [members])
                ...
                <>
                  <Box
                    ...
                    <Box
                      ...
                      <Checkbox checked={selectedName.indexOf(name.id) >= 0} />
                      <ListItemText primary={name.checkBoxLabel} />
                    </Box>
                    <Box>
                      <Typography
                        variant="subtitle2"
                        sx={{ marginLeft: '43px' }}>
                        {name?.checkBoxSubtitle}
                      </Typography>
                    </Box>
                  </Box>
                  <Box>
          ...
          <Box marginY={2}>
            {selectedName?.map((value) => (
              <Chip
                ...
                label={
                  names[
                    names.findIndex((item: { id: string }) => item.id === value)
                  ]?.label
                }
                sx={{
                  fontSize: theme.typography.subtitle2.fontSize,
